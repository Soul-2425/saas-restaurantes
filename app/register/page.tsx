  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const res = await fetch('/api/auth/register', {
        ...
        body: JSON.stringify({ email, password, nombre, restauranteNombre }),
      })
      const { data, error: apiError } = await res.json()
      
      if (!res.ok || apiError) { 
        setError(apiError ?? 'Error al crear la cuenta')
        return 
      }
      
      if (data?.restaurante_id) {
        localStorage.setItem('rg_sucursal', data.restaurante_id)
      }
      ...
      <div style={{
        ...
        <div style={{ marginBottom: '3rem' }}>
          ...
          <h1 style={{
            ...
            Únete a la <br />
            <span style={{ color: 'var(--gold-light)' }}>Revolución</span>
          </h1>
          <p style={{ color: 'var(--text-2)', fontSize: '1rem', maxWidth: 360, lineHeight: 1.7 }}>
            Crea tu primer Tenant y comienza a operar tu negocio con la mayor precisión de la industria.
          </p>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.9rem' }}>
          {[
            { icon: '⭐', label: 'Dashboard operativo inmediato' },
            { icon: '🏢', label: 'Crea tu propio Workspace aislado' },
            { icon: '🚀', label: 'Preparado para escalar a múltiples sucursales' },
          ].map(({ icon, label }) => (
        ...
        <div className="card-glass animate-fade-up" style={{ width: '100%', padding: '2.5rem' }}>
          ...
          <div style={{ marginBottom: '2rem' }}>
            ...
            <p style={{ color: 'var(--text-3)', fontSize: '0.875rem' }}>
              Empieza gratis con tu primer local
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            <div style={{ display: 'flex', gap: '1rem' }}>
              <div style={{ flex: 1 }}>
                ...
                <input
                  ...
                  style={{ background: 'var(--surface-2)', width: '100%' }}
                />
              </div>
              <div style={{ flex: 1 }}>
                <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 500, color: 'var(--text-2)', marginBottom: '0.4rem' }}>
                ...
                <input
                  type="text" required
                  value={restauranteNombre} onChange={e => setRestauranteNombre(e.target.value)}
            ...
            <button type="submit" disabled={loading} className="btn btn-red" style={{ width: '100%', marginTop: '0.5rem', height: 44 }}>
              {loading
                ? <Loader2 size={16} style={{ animation: 'spin 0.8s linear infinite' }} />
                : <><span>Comenzar ahora</span><ArrowRight size={16} /></>
              }
            </button>
          </form>
          ...
          <p style={{ textAlign: 'center', fontSize: '0.825rem', color: 'var(--text-3)' }}>
            ¿Ya tienes cuenta?{' '}
            <a href="/login" style={{ color: 'var(--gold-light)', fontWeight: 500 }}>
              Iniciar sesión
            </a>
          </p>
        </div>
